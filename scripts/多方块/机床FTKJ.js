const 工厂 = require("特定/工厂");//需使用定义这个工厂type的文件,这两个名字（JiChuang,钢钛合金冶炼厂）不能相同（存疑）,这段代码会在第56行用到,需要修改
const coord=[-1,2,0,2,1,2,2,2,-1,1,0,1,1,1,2,1,-1,0,1,0,2,1,-1,-1,0,-1,1,-1,2,-1]//方块位置,以工厂核心的中心（核心边长为奇数）/中心偏左下角（核心边长为偶数） 为原点
const list=[1,2,2,1,2,4,3,2,2,4,2,1,2,2,1]//以最右上角为开始,数字项类似宏定义,数字见下
function bbbb(a){//定义
	var b
	a=a.toString()
if (a=="原版拓展-钢制架构"){b=1}
...
//~~~~~~~~~~~~~~
var auto = false
const 钢钛合金冶炼厂控制器 = extend(GenericCrafter, '钢钛合金冶炼厂控制器', {});
钢钛合金冶炼厂控制器.configurable = true;
钢钛合金冶炼厂控制器.buildType = prov(() => {
	var other
	var ii=0
	var d
	var bb
	...
	return new JavaAdapter(GenericCrafter.GenericCrafterBuild, {
		buildConfiguration(table){
			...
			var dialog = new BaseDialog("钢钛合金冶炼厂控制器");
			dialog.cont.pane(table => {
				table.add("多方块结构:钢钛合金冶炼厂")
				table.row()
				table.add(" ")
				table.row()
		...
		updateTile(){
			...
			if (cc==1 && e==1){
Vars.world.tile(this.tileX(), this.tileY()).setBlock(工厂.钢钛合金冶炼厂,this.team);}
			if (other!=null){
				bb=bbbb(other.block);
